...
$(document).ready(function () {
    ...
    $($('.checkbox label')).click(function (e) {
        ...
        if ($(this).parent().find('input[type="checkbox"]').is(':checked')) {
            $(this).parent().parent().find('button[type="submit"]').addClass('disable')
            $(this).parent().find('input[type="checkbox"]').prop('checked', false);
        }
        else {
            $(this).parent().parent().find('button[type="submit"]').removeClass('disable')
    ...
    if ($(".slider").length) {
        ...
        $('.slider .logo span').css('background', 'url(' + logo + ') no-repeat 50%/contain');
            ...
            if ($(window).width() > 900) {
                $(".split").each(function () {
                    ...
                    var timeline = gsap.timeline({
                        ...
                        .from($(this).find('span'), { yPercent: 100, duration: 2, ease: "power4.out" })
                });
            }
        });
    }
        ...
        $(window).on('load resize', function () {
            if ($(window).width() > 900) {
                $(".part").each(function () {
                    var timeline = gsap.timeline({
                        scrollTrigger: {
                            trigger: $(this).find('.part2'),
                            scroller: '.wrapper',
                            start: 'top center',
                            ease: "none",
                            scrub: true,
                            end: "+=" + $(this).find('.part2').height() + ""
                        }
                    })
                        .to(this, {
                            background: '#26262d',
